...
function getBearerToken() {
    ...
    // Método 1: REDIRECT_HTTP_AUTHORIZATION (Ubuntu con .htaccess rewrite)
    if (isset($_SERVER['REDIRECT_HTTP_AUTHORIZATION'])) {
        $token = str_replace('Bearer ', '', $_SERVER['REDIRECT_HTTP_AUTHORIZATION']);
    }
    // Método 2: HTTP_AUTHORIZATION (Windows/XAMPP normal)
    elseif (isset($_SERVER['HTTP_AUTHORIZATION'])) {
        $token = str_replace('Bearer ', '', $_SERVER['HTTP_AUTHORIZATION']);
    }
    // Método 3: getallheaders() (algunos servidores)
    ...
    // Método 4: apache_request_headers() (Apache)
...
// Debug
file_put_contents(__DIR__ . '/../data/debug.log', 
    "=== TOKEN CHECK ===\n" .
    "Token: " . $token . "\n" .
    "REDIRECT: " . ($_SERVER['REDIRECT_HTTP_AUTHORIZATION'] ?? 'NO') . "\n" .
    "HTTP_AUTH: " . ($_SERVER['HTTP_AUTHORIZATION'] ?? 'NO') . "\n" .
    "==================\n",
    FILE_APPEND
);
